...
struct RVAData 
{
    std::unordered_map<uint32_t, uintptr_t> addr;   // Map of runtime version to RVA.
...
public:
    ...
    static void UpdateSingle(std::shared_ptr<RVAData> rvaData, uint32_t runtimeVersion = 0) {
        if (SHOW_ADDR != 1 && rvaData->addr.count(runtimeVersion) > 0) {
            ...
            if (rvaData->sig) {
                ...
                if (rvaData->indirectOffset != 0) {
                    int32_t rel32 = 0;
                    RVAUtils::ReadMemory(rvaData->effectiveAddress + rvaData->indirectOffset, &rel32, sizeof(int32_t));
                    ...
                    _MESSAGE("RVA: 0x%08X", rvaData->effectiveAddress - (uintptr_t)GetModuleHandle(NULL));
                    _MESSAGE("---");
                #endif
    ...
    static uintptr_t GetEffectiveAddress(uintptr_t rva) {
        return (uintptr_t)GetModuleHandle(NULL) + rva;
    }
...
{
public:
    using AddressMap = std::unordered_map<uint32_t, uintptr_t>;
    ...
    void Resolve(uint32_t runtimeVersion) {
